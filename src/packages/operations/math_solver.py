...
class MathSolver():
    ...
    def linear_solver(self):
        ...
        index_var_dict = self.variable_search(
            string_equation, check_list)
        unknown_var = self.is_valid_equation(string_equation, index_var_dict)
        if not unknown_var:
        ...
        try:
            ...
            left_hand = self.simple_solver(
                self.special_operator_value({}, left_hand)[1])
            right_hand = self.simple_solver(
                self.special_operator_value({}, right_hand)[1])
            final_ans = self.twin_solver(left_hand, right_hand, unknown_var)
        return final_ans

    def simple_solver(self, sub_string: str):
        sub_string = self.basic_solver(sub_string)
        sub_string = self.complementary_solver(sub_string)
        return sub_string
    ...
    def twin_solver(self, left_hand: str, right_hand: str, target_variable: str):
        """Function to resolve unknown"""
        left_hand = self.strip_trimming(left_hand)
        right_hand = self.strip_trimming(right_hand)
        if target_variable in right_hand:
            final_ans = self.equation_solver(
    ...
    def equation_solver(self, var_side, value_side, target_variable):
        return None

    def complementary_solver(self, string_equation: str):
        string_function = self.string_trimming(string_equation)
    ...
    def strip_trimming(self, string_expression: str):
        if string_expression[0] == "(" and string_expression[-1] == ")":
            return string_expression[1:-1]
        return string_expression

    def variable_search(self, string_equation: str, check_list: list):
        # trimming string
    ...
    def special_operator_value(self, orig_sub_dict: dict, sub_expression: str):
    ...
    def variable_substitution(self, string_equation: str, sub_index_dict: dict):
        # regex catching
        orig_sub_dict = {}
            ...
            for pair in pairs:
                ...
                for key_sub, value_sub in orig_sub_dict.items():
                    sub_expression = sub_expression.replace(
                        key_sub, value_sub)
                orig_sub_dict, sub_expression = self.special_operator_value(
                    orig_sub_dict, sub_expression)
                final_sub = self.simple_solver(sub_expression[1:-1])
                orig_sub_dict[sub_expression] = final_sub
        return orig_sub_dict


test = MathSolver(
    "(T1/T2)^(gamma) = (P1/P2)**(gamma)", t1=5, t2=5, gamma=1.25, p2=3)
